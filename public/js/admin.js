...
try { history.scrollRestoration = 'manual'; } catch(e){}   // views decide their own scroll: a new one starts at the top
function setView(h, replace){
  h = h || '';
  const changed = h !== location.hash.slice(1);
...
function cardView(v){
  setView('card=' + v.id);
  $('main').innerHTML = venueHead(v, 'card') + '<div id="cardHost"></div>';
  $('back').onclick = ()=> home();
  ...
  mountCardDesigner($('cardHost'), v, {
    ...
    save: async card => {
      const { data, error } = await rpc('admin_set_card', { p_id: v.id, p_card: card });
      if (error || !data || data.ok === false) throw new Error((data && data.error) || 'refused');
      const live = venueById(v.id); if (live) live.card = data.card || null;
